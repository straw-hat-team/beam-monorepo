defmodule OnePiece.Nacha.PosEntryDetail do
  import NimbleParsec
  import OnePiece.Nacha.Helpers
  import OnePiece.Nacha.PosAddenda, only: [pos_addenda: 0]
  ...
  def pos_entry_detail do
    ...
    |> individual_name()
    |> ascii_string([], 2)
    |> label("Card Transaction Code")
    |> addenda_record_indicator()
    |> trace_number()
    |> eol()
    |> optional(pos_addenda())
    |> post_traverse({__MODULE__, :post_traverse, []})
    |> label("POS Entry Detail")
  end
end
